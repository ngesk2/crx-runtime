...
async def get_infrastructure_status():
    ...
    services.append(ServiceStatus(
        ...
        configuration_source="environment",
        dependencies=["docker"],
        metrics={"provider": inference_adapter.provider.value if 'inference_adapter' in locals() else "unknown"}
    ))
...
async def search_memory(query: str, limit: int = 10):
    ...
    # This will use the constitutional_search module
    # For now, return mock data
    return {
        "query": query,
        "results": [],
        "message": "Search endpoint - requires constitutional_search integration"
    }
...
async def get_inference_models():
    ...
    try:
        ...
        if models:
            ...
            return {"status": "unhealthy", "error": f"Ollama API returned {response.status_code}"}
    except Exception as e:
        return {"status": "unhealthy", "error": str(e)}
...
async def get_backup_status():
    """Get backup status."""
    # This will check rclone configuration and backup health
    # For now, return mock data
    return {
        "status": "unknown",
        "last_backup": None,
        "backup_path": "gdrive:PING_BACKUPS",
        "retention_policy": "7 daily, 4 weekly, 12 monthly",
        "schedule": "manual",  # Needs to be automated
        "message": "Backup automation not yet implemented"
    }
...
# Ollama Harness Integration
@app.get("/ollama/harness/validate")
async def validate_ollama_harness():
    """Validate Ollama harness model routing."""
    from ollama_harness import get_harness, ModelRegistry, ModelType
    
    harness = get_harness()
    
    return {
        "available_models": harness.available_models,
        "embedding_models": harness.get_embedding_models(),
        "chat_models": harness.get_chat_models(),
        "tool_models": harness.get_tool_models(),
        "primary_model": os.getenv('PING_PRIMARY_MODEL', 'qwen2.5-coder:7b'),
        "registry_status": "operational"
    }


@app.post("/ollama/harness/test")
async def test_ollama_harness():
    """Test Ollama harness with model routing validation."""
    from ollama_harness import get_harness, ModelUsageError
    ...
    try:
        embedding = harness.generate_embedding("test")
        results.append({
            "test": "embedding_with_correct_model",
            "status": "success",
    ...
    # Test chat with wrong model (should fail)
    try:
        harness.chat([{"role": "user", "content": "test"}], model="nomic-embed-text")
        results.append({
            "test": "chat_with_embedding_model",
            "status": "failed",
            "error": "Should have raised ModelUsageError"
        })
    except ModelUsageError as e:
        results.append({
            "test": "chat_with_embedding_model",
            "status": "success",
            "error": str(e)
...
async def get_model_capabilities():
    ...
    try:
        ...
        if os.path.exists(MODEL_CAPABILITIES_PATH):
            ...
            # Fallback to live provider query through authority
            import sys
            sys.path.append(os.path.join(os.path.dirname(__file__), 'runtime', 'adapters'))
            from inference_adapter import get_inference_adapter
            inference_adapter = get_inference_adapter()
...
async def get_continuity_status():
    ...
    # Ollama harness validation
    try:
        from ollama_harness import get_harness
        harness = get_harness()
        harness_healthy = len(harness.available_models) > 0
        status["components"]["ollama_harness"] = {
            "status": "healthy" if harness_healthy else "unhealthy",
            "message": f"Ollama harness operational with {len(harness.available_models)} models" if harness_healthy else "Ollama harness failed to initialize"
        }
    except Exception as e:
        status["components"]["ollama_harness"] = {
            "status": "unhealthy",
            "message": f"Ollama harness error: {str(e)}"
        }
